...
module Outboxer
  RSpec.describe Publisher do
    describe '.publish' do
      ...
      context 'when metrics submit interval passes' do
        let(:metrics_statsd) { double }
        let(:metrics_submit_interval) { 0.5 }
        let(:metrics_tags) { ["env:test"] }

        before do
          allow(metrics_statsd).to receive(:gauge)
          allow(metrics_statsd).to receive(:batch).and_yield
        end
        ...
        it 'sends metrics to statsd' do
          Publisher.publish(
            ...
            metrics_statsd: metrics_statsd,
            metrics_submit_interval: metrics_submit_interval,
          ...
          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.count", 0, tags: ["env:test","status:queued"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.latency", anything, tags: ["env:test","status:queued"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.count", 0, tags: ["env:test","status:dequeued"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.latency", anything, tags: ["env:test","status:dequeued"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.count", 0, tags: ["env:test","status:publishing"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.latency", anything, tags: ["env:test","status:publishing"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.count", 0, tags: ["env:test","status:published"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.latency", anything, tags: ["env:test","status:published"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.count", 0, tags: ["env:test","status:failed"]
          ).at_least(:once)

          expect(metrics_statsd).to have_received(:gauge).with(
            "outboxer.messages.latency", anything, tags: ["env:test","status:failed"]
          ).at_least(:once)
        end
      end
