  ...
  RSpec.describe MessageService do
    describe ".delete_all" do
      ...
      context "when status is failed" do
        before do
          MessageService.delete_all(status: MessageService::Status::FAILED, batch_size: 1)
        end
      ...
      context "when status is buffered" do
        before do
          MessageService.delete_all(status: MessageService::Status::BUFFERED, batch_size: 1)
        end
      ...
      context "when status is publishing" do
        before do
          MessageService.delete_all(status: MessageService::Status::PUBLISHING, batch_size: 1)
        end
      ...
      context "when status is published" do
        context "when older than nil" do
          before do
            MessageService.delete_all(status: MessageService::Status::PUBLISHED, batch_size: 1)
          end
      ...
      context "when status not passed" do
        before do
          MessageService.delete_all(batch_size: 1)
        end
      ...
      context "when status is nil" do
        before do
          MessageService.delete_all(status: nil, batch_size: 1)
        end
