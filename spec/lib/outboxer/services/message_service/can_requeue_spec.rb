  ...
  RSpec.describe MessageService do
    describe ".can_requeue?" do
      ...
      context "when status is queued" do
        it "returns true" do
          expect(MessageService.can_requeue?(status: MessageService::Status::QUEUED)).to eq false
        end
      end

      context "when status is buffered" do
        it "returns true" do
          expect(MessageService.can_requeue?(status: MessageService::Status::BUFFERED)).to eq true
        end
      end

      context "when status is publishing" do
        it "returns true" do
          expect(MessageService.can_requeue?(status: MessageService::Status::PUBLISHING)).to eq true
        end
      end

      context "when status is failed" do
        it "returns true" do
          expect(MessageService.can_requeue?(status: MessageService::Status::FAILED)).to eq true
        end
      end

      context "when status is nil" do
        it "returns false" do
          expect(MessageService.can_requeue?(status: nil)).to eq false
        end
      end
    end
  end
