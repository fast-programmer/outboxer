        ...
        it "sorts messages by status in ascending order" do
          expect(
            Messages
              ...
              .map { |message| message[:status] }).to eq(%i[dequeued failed publishing queued
                                                            queued])
        end
        ...
        it "sorts messages by status in descending order" do
          expect(
            Messages
              ...
              .map { |message| message[:status] }).to eq(%i[queued queued publishing failed
                                                            dequeued])
        end
      end
      ...
      context "with sort by messageable" do
        it "sorts messages by messageable in ascending order" do
          expect(
            ...
            end).to eq([%w[Event 1], %w[Event 2], %w[Event 3], %w[Event 4],
                        %w[Event 5]])
        end
        ...
        it "sorts messages by messageable in descending order" do
          expect(
            Messages
              ...
              [message[:messageable_type],
               message[:messageable_id]]
            end).to eq([%w[Event 5], %w[Event 4], %w[Event 3], %w[Event 2],
                        %w[Event 1]])
        end
      end
